...
type Config struct {
	...
	Prefix              string // path within container
	AccessKey           string // auth: shared key
	SASToken            string // auth: SAS
	Endpoint            string // full service URL override (path-style, e.g. Azurite/private deployments)
	EndpointSuffix      string // overrides env-derived suffix
	EnvironmentName     string // AzurePublicCloud (default), AzureUSGovernmentCloud, AzureChinaCloud, AzureGermanCloud
	BufferSize          int    // upload block size, default 8MiB, min 1KiB
	MaxBuffers          int    // upload concurrency, default 4, min 1
	TryTimeout          int    // minutes, default 5
	BlobStoreAPIVersion string // optional x-ms-version override
}
